...
class FrameCameraController:
    ...
    def __init__(self, status_callback=None):
        ...
        self.preview_callback = None  # For embedded preview
    ...
    def _preview_worker(self):
        """Preview worker thread - uses embedded preview with direct callback"""
        ...
        while not self.exit_flag and self.show_preview:
            try:
                ...
                # Send frame to embedded preview using direct callback
                if self.preview_callback and hasattr(self, 'external_preview_active') and not self.external_preview_active:
                    # Schedule GUI update on main thread
                    if hasattr(self.preview_callback, '__self__'):  # Check if it's a bound method
                        root = getattr(self.preview_callback.__self__, 'root', None)
                        if root:
                            root.after_idle(lambda f=frame: self.preview_callback(f))
        ...
        if self.preview_callback:
            if hasattr(self.preview_callback, '__self__'):
                root = getattr(self.preview_callback.__self__, 'root', None)
                if root:
                    root.after_idle(lambda: self.preview_callback(None))
...
class EventCameraController:
    ...
    def _visualization_thread(self):
        """Thread for event visualization - uses embedded preview with direct callback"""
        try:
            event_frame_gen = PeriodicFrameGenerationAlgorithm(
                sensor_width=self.width, sensor_height=self.height, 
                fps=25, palette=ColorPalette.Dark
            ...
            def on_cd_frame_cb(ts, cd_frame):
                # Send frame to embedded preview using direct callback
                if self.preview_callback and not self.external_preview_active:
                    # Schedule GUI update on main thread
                    if hasattr(self.preview_callback, '__self__'):  # Check if it's a bound method
                        root = getattr(self.preview_callback.__self__, 'root', None)
                        if root:
                            root.after_idle(lambda f=cd_frame: self.preview_callback(f))
...
class DualCameraGUI:
    ...
    def __init__(self):
        ...
        self.frame_camera.set_preview_callback(self.update_frame_preview)
        self.event_camera.set_preview_callback(self.update_event_preview)
    ...
    def update_frame_preview(self, frame):
        """Update frame camera preview in GUI - thread-safe using after_idle"""
        try:
    ...
    def update_event_preview(self, frame):
        """Update event camera preview in GUI - thread-safe using after_idle"""
        try:
            if frame is not None and PIL_AVAILABLE:
                # Get preview label dimensions
