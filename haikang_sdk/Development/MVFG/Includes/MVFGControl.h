 ...
 *  @param  hPort                   [IN]            设备句柄/采集卡句柄
 *  @param  strFileName             [IN]            属性文件名，不支持中文路径
 *  @param  stNodeErrorList         [IN OUT]        错误信息列表，由用户在外部申请并由内部填充数据，该参数允许填null代表用户不关心导入时的错误信息
 *  @return 成功，返回MV_FG_SUCCESS；错误，返回错误码
 *  @remarks 部分节点导入失败时，接口返回MV_FG_SUCCESS，通过错误信息列表中stNodeError获取出错节点及失败原因
 ...
 *  @return Return MV_FG_SUCCESS on success, and return Error Code on failure.
 *  @remarks When some nodes fail to load, the interface returns MV_FG_SUCCESS. \n
 *           The error node and the reason for the failure are obtained through stNodeError in the error message list.
 ************************************************************************/
MV_FGCTRL_API int __stdcall MV_FG_FeatureLoadEx(IN PORTHANDLE hPort, IN const char* strFileName, IN OUT MV_FG_NODE_ERROR_LIST* pstNodeErrorList);
