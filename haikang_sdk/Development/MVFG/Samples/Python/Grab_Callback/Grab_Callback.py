...
stFrameInfo = POINTER(MV_FG_BUFFER_INFO)
FrameInfoCallBack = winfun_ctype(None, stFrameInfo, c_void_p)

def image_callback(pFrameInfo, pUser):
        stFrameInfo = cast(pFrameInfo, POINTER(MV_FG_BUFFER_INFO)).contents
        if stFrameInfo:
            print ("FrameNum[%d]: Width[%d], Height[%d]" % (stFrameInfo.nFrameID, stFrameInfo.nWidth, stFrameInfo.nHeight))
...
if __name__ == '__main__':
    ...
    for i in range(0, 1):
        ...
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_UpdateInterfaceList error, ret[0x%x]" % ret)
            break
        ...
        if MV_FG_SUCCESS != ret or 0 == nInterfaceNum.value:
            print("Warning: MV_FG_GetNumInterfaces error, ret[0x%x]" % ret)
            break
        ...
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_OpenInterface error, ret[0x%x]" % ret)
            break

        # ch: 枚举采集卡上的相机 | en:Enum camera of interface
        ret = Iface.UpdateDeviceList(bChanged)
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_UpdateDeviceList error, ret[0x%x]" % ret)
            break
        ...
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_GetNumDevices error, ret[0x%x]" % ret)
            break
        ...
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_OpenDevice error, ret[0x%x]" % ret)
            break
        ...
        if MV_FG_SUCCESS != ret or 0 == nStreamNum.value:
            print("Warning: MV_FG_GetNumStreams error, ret[0x%x]" % ret)
            break

        # ch:打开流通道(目前只支持单个通道) | en:Open stream(Only a single stream is supported now)
        ret = Strm.OpenStream(Dev, 0)
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_OpenStream error, ret[0x%x]" % ret)
            break

        # ch:设置SDK内部缓存数量 | en:Set internal buffer number
        ret = Strm.SetBufferNum(BUFFER_NUMBER)
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_SetBufferNum error, ret[0x%x]" % ret)
            break

        # ch: 注册帧缓存信息回调函数 | en:Register frame info callback
        ret = Strm.RegisterFrameCallBackEx(CALL_BACK_FUN, None, True)
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_RegisterFrameCallBackEx error, ret[0x%x]" % ret)
            break
    ...
    if Strm._STREAMHANDLE.value is not NULL:
        ...
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_CloseStream error, ret[0x%x]" % ret)
    ...
    if Dev._DEVHANDLE.value is not NULL:
        ret = Dev.CloseDevice()
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_CloseDevice error, ret[0x%x]" % ret)
        Dev.DEVHANDLE = None
    ...
    if Iface._IFHANDLE.value is not NULL:
        ...
        if MV_FG_SUCCESS != ret:
            print("Warning: MV_FG_CloseInterface error, ret[0x%x]" % ret)
        Iface.IFHANDLE = None
