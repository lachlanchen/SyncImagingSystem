...
[PS] Sample programs that need to save files need to be executed with administrator privileges \
     ...
     Old FirmWare may not support stream trigger from interface. For example, GS1002F support stream trigger from MV_GS1002F_HS V1.0.0_230830.
*/
#include <stdio.h>
#include <Windows.h>
#include <process.h>
...
int main()
{
    ...
    do
    {
        ...
        bool bChanged = false;
        nRet = MV_FG_UpdateInterfaceList(MV_FG_XoF_INTERFACE, &bChanged);
        if (MV_FG_SUCCESS != nRet)
        {
