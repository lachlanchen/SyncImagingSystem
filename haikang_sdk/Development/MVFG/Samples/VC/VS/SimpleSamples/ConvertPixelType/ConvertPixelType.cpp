...
bool IsColorPixelFormat(MV_FG_PIXEL_TYPE enPixelType)
{
    ...
    case MV_FG_PIXEL_TYPE_BayerRBGG8:
        ...
        while (!g_bExit)
        {
            ...
            if ((stFrameInfo.pBuffer) && (0 < stFrameInfo.nFilledSize) && (SAVE_IMAGE_NUM > nSaveNum))
            {
                ...
                if (IsColorPixelFormat(stFrameInfo.enPixelType))
                {
                    ...
                    sprintf_s(szFileName, FILE_NAME_LEN, "After%d_%04d%02d%02d%02d%02d%02d%03d.rgb", ++nSaveNum, sys.wYear, sys.wMonth,
                ...
                else if (IsMonoPixelFormat(stFrameInfo.enPixelType))
                {
                    ...
                    sprintf_s(szFileName, FILE_NAME_LEN, "After%d_%04d%02d%02d%02d%02d%02d%03d.gray", ++nSaveNum, sys.wYear, sys.wMonth,
                        sys.wDay, sys.wHour, sys.wMinute, sys.wSecond, sys.wMilliseconds);
                }
                else
                {
