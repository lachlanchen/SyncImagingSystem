...
namespace BasicDemo
{
    public partial class Form1 : Form
    {
        ...
        private void EnableControls()
        {
            ...
            this.btnSaveBmp.Enabled = m_bIsGrabbing;
            this.btnSaveJpg.Enabled = m_bIsGrabbing;
        }
                    ...
                    if (CErrorCode.MV_FG_SUCCESS == nRet)
                    {
                        ...
                        lock (m_LockForSaveImage)
                        {
                            if (IntPtr.Zero == m_pDataBuf || m_nDataBufSize < stFrameInfo.nFilledSize)
                            {
                                ...
                                m_pDataBuf = Marshal.AllocHGlobal(new IntPtr(stFrameInfo.nFilledSize));
                                if (IntPtr.Zero == m_pDataBuf)
                                {
        ...
        private int SaveImage(SAVE_IAMGE_TYPE enSaveImageType)
        {
            ...
            lock (m_LockForSaveImage)
            {
                ...
                if (IntPtr.Zero == m_pSaveImageBuf || m_nSaveImageBufSize < nMaxImageLen)
                {
                    ...
                    m_pSaveImageBuf = Marshal.AllocHGlobal((int)nMaxImageLen);
                    if (IntPtr.Zero == m_pSaveImageBuf)
                    {
        ...
        {
            int nRet = SaveImage(SAVE_IAMGE_TYPE.Image_Jpeg);
            if (CErrorCode.MV_FG_SUCCESS != nRet)
            {
                MessageBox.Show("Save JPEG failed, ErrorCode:" + nRet.ToString("X"));
                return;
            }

            MessageBox.Show("Save JPEG succeed");
        }
