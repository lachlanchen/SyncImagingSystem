...
namespace GrabImage_Callback
{
    class CImageDelegate
    {
        public CStream.ImageDelegate ImageDelegate { get; private set; }

        public CImageDelegate()
        {
            ImageDelegate = ImageCallbackFunc;
        }

        // ch:帧信息回调函数 | en:Frame info callback
        void ImageCallbackFunc(ref MV_FG_BUFFER_INFO stBufferInfo, IntPtr pUser)
        {
    ...
    class GrabImage_Callback
    {
        static void Main(string[] args)
        {
            ...
            CImageDelegate cImageDelegate = new CImageDelegate();  // ch:帧信息回调函数委托类 | en:Frame info callback delegate instance
            ...
            do
            {
                ...
                nRet = cStream.RegisterImageCallBack(cImageDelegate.ImageDelegate, IntPtr.Zero, true);
                if (CErrorCode.MV_FG_SUCCESS != nRet)
                {
