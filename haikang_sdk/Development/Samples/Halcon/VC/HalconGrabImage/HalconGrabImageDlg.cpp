...
int CHalconGrabImageDlg::OpenDevice(void)
{
    ...
    if (MV_GENTL_CXP_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType
        || MV_GENTL_CAMERALINK_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType
        || MV_GENTL_XOF_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType)
    {
        nRet = OpenFrameGrabberGenICamTL(nIndex);
    }
    else
    {
        nRet = OpenFrameGrabberMVision(nIndex);
    }
...
void CHalconGrabImageDlg::OnBnClickedEnumButton()
{
    ...
    int nRet = MV_CC_EnumDevicesEx2(MV_GIGE_DEVICE | MV_USB_DEVICE | MV_GENTL_CAMERALINK_DEVICE
        | MV_GENTL_CXP_DEVICE | MV_GENTL_XOF_DEVICE | MV_GENTL_GIGE_DEVICE, &stDevList, NULL, SortMethod_SerialNumber);
    if (MV_OK != nRet)
    {
    ...
    for (i = 0; i < stDevList.nDeviceNum; i++)
    {
        ...
        if (pDeviceInfo->nTLayerType == MV_GIGE_DEVICE || pDeviceInfo->nTLayerType == MV_GENTL_GIGE_DEVICE)
        {
        ...
        else if (pDeviceInfo->nTLayerType == MV_GENTL_CAMERALINK_DEVICE)
        {
            ...
            {
                DWORD dwLenUserName = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)(pDeviceInfo->SpecialInfo.stCMLInfo.chUserDefinedName), -1, NULL, 0);
                pUserName = new wchar_t[dwLenUserName];
                MultiByteToWideChar(CP_ACP, 0, (LPCSTR)(pDeviceInfo->SpecialInfo.stCMLInfo.chUserDefinedName), -1, pUserName, dwLenUserName);
            }
            else
            {
                char strUserName[256];
                sprintf_s(strUserName, "%s %s (%s)", pDeviceInfo->SpecialInfo.stCMLInfo.chManufacturerInfo,
            ...
            strMsg.Format(_T("[%d]GenTL CML:  %s"), i, pUserName);
            if (NULL != pUserName)
            {
                delete(pUserName);
                pUserName = NULL;
        ...
        else if (pDeviceInfo->nTLayerType == MV_GENTL_CXP_DEVICE)
        {
            ...
            {
                DWORD dwLenUserName = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)(pDeviceInfo->SpecialInfo.stCXPInfo.chUserDefinedName), -1, NULL, 0);
                pUserName = new wchar_t[dwLenUserName];
                MultiByteToWideChar(CP_ACP, 0, (LPCSTR)(pDeviceInfo->SpecialInfo.stCXPInfo.chUserDefinedName), -1, pUserName, dwLenUserName);
            }
            else
            {
                char strUserName[256];
                sprintf_s(strUserName, "%s %s (%s)", pDeviceInfo->SpecialInfo.stCXPInfo.chManufacturerInfo,
            ...
            strMsg.Format(_T("[%d]GenTL CXP:  %s"), i, pUserName);
            if (NULL != pUserName)
            {
                delete(pUserName);
                pUserName = NULL;
        ...
        else if (pDeviceInfo->nTLayerType == MV_GENTL_XOF_DEVICE)
        {
            ...
            strMsg.Format(_T("[%d]GenTL XOF:  %s"), i, pUserName);
            if (NULL != pUserName)
            {
                delete(pUserName);
                pUserName = NULL;
...
void CHalconGrabImageDlg::OnBnClickedStartGrabbingButton()
{
    ...
    {
        Herror  error_num = except.err;
        MessageBox(TEXT("grab_image_start fail"), TEXT("warning"), MB_OK | MB_ICONWARNING);
        return;
    }
...
int CHalconGrabImageDlg::OpenFrameGrabberMVision(int nIndex)
{
    char strSerial[64] = { 0 };
    if (MV_GIGE_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType || MV_GENTL_GIGE_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType)
    {
        strcpy(strSerial, "GEV:");
        memcpy(strSerial + 4, m_pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.chSerialNumber,
            sizeof(m_pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.chSerialNumber));
    }
    else if (MV_USB_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType)
    {
        strcpy(strSerial, "U3V:");
        memcpy(strSerial + 4, m_pDeviceInfo[nIndex]->SpecialInfo.stUsb3VInfo.chSerialNumber,
...
int CHalconGrabImageDlg::OpenFrameGrabberGenICamTL(int nIndex)
{
    char strSerial[64] = { 0 };
    if (MV_GENTL_CXP_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType)
    {
        memcpy(strSerial, m_pDeviceInfo[nIndex]->SpecialInfo.stCXPInfo.chDeviceID,
            sizeof(m_pDeviceInfo[nIndex]->SpecialInfo.stCXPInfo.chDeviceID));
    }
    else if (MV_GENTL_CAMERALINK_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType)
    {
        memcpy(strSerial, m_pDeviceInfo[nIndex]->SpecialInfo.stCMLInfo.chDeviceID,
            sizeof(m_pDeviceInfo[nIndex]->SpecialInfo.stCMLInfo.chDeviceID));
    }
    else if (MV_GENTL_XOF_DEVICE == m_pDeviceInfo[nIndex]->nTLayerType)
    {
        memcpy(strSerial, m_pDeviceInfo[nIndex]->SpecialInfo.stXoFInfo.chDeviceID,
            sizeof(m_pDeviceInfo[nIndex]->SpecialInfo.stXoFInfo.chDeviceID));
    }
    else
    {
        return MV_E_PARAMETER;
    ...
    try
    {
        open_framegrabber("GenICamTL", 0, 0, 0, 0, 0, 0, "progressive", -1, "default", -1,
            "false", "default", (char*)strSerial, 0, -1, &m_htupHalconHandle);
    }
    catch (HException &except)
    {
        Herror  error_num = except.err;
