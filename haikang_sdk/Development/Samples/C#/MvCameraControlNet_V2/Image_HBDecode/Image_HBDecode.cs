     ...
     | DeviceTLayerType.MvGenTLGigEDevice | DeviceTLayerType.MvGenTLCXPDevice | DeviceTLayerType.MvGenTLCameraLinkDevice | DeviceTLayerType.MvGenTLXoFDevice;
            ...
            try
            {
                ...
                if (MvError.MV_OK == result)
                {
                    ...
                    string outputFilePath = string.Format("HBDecode_w{0}_h{1}_{2}.raw", frameDecode.Image.Width, frameDecode.Image.Height, frameDecode.Image.PixelType);
                    using (FileStream fs = new FileStream(outputFilePath, FileMode.CreateNew))
                    {
                        fs.Write(frameDecode.Image.PixelData, 0, frameDecode.Image.PixelData.Length);
                    }
