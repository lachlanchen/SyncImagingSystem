    ...
    class Events_Camera
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                ret = device.Parameters.SetEnumValueByString("EventSelector", "ExposureEnd");
                if (ret != MvError.MV_OK)
                {
                    Console.WriteLine("Set EventSelector failed!:{0:x8}", ret);
                    return;
                }

                ret = device.Parameters.SetEnumValueByString("EventNotification", "On");
                if (ret != MvError.MV_OK)
                {
