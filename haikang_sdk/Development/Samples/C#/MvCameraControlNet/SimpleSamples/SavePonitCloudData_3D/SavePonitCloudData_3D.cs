        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                if (stDevInfo.nTLayerType == MyCamera.MV_GIGE_DEVICE)
                {
                    ...
                    if (nPacketSize > 0)
                    {
                        nRet = device.MV_CC_SetIntValueEx_NET("GevSCPSPacketSize", Convert.ToInt32(nPacketSize));
                ...
                nRet = device.MV_CC_GetIntValueEx_NET("PayloadSize", ref stParam);
                if (MyCamera.MV_OK != nRet)
                {
                    Console.WriteLine("Get PayloadSize failed:{0:x8}", nRet);
                    break;
                }
                UInt32 nPayloadSize = (UInt32)stParam.nCurValue;
