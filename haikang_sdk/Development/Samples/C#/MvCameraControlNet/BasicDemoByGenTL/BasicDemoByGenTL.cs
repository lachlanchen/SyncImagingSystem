...
namespace BasicDemoByGenTL
{
    public partial class Form1 : Form
    {
        ...
        private void DeviceListAcq()
        {
            ...
            {
                MyCamera.MV_GENTL_DEV_INFO_EX device = (MyCamera.MV_GENTL_DEV_INFO_EX)Marshal.PtrToStructure(m_stDeviceList.pDeviceInfo[i], typeof(MyCamera.MV_GENTL_DEV_INFO_EX));
                string strUserDefinedName = "";

                if ((device.chUserDefinedName.Length > 0) && (device.chUserDefinedName[0] != '\0'))
                {
                    if (MyCamera.IsTextUTF8(device.chUserDefinedName))
                    {
                        strUserDefinedName = Encoding.UTF8.GetString(device.chUserDefinedName).TrimEnd('\0');
                    }
                    else
                    {
                        strUserDefinedName = Encoding.Default.GetString(device.chUserDefinedName).TrimEnd('\0');
                    }

                    cmbDeviceList.Items.Add("Dev: " + strUserDefinedName + " (" + device.chSerialNumber + ")");
                }
                else
                {
