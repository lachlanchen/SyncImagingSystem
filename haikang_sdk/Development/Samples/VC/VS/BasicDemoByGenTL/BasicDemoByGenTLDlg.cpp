...
void CBasicDemoByGenTLDlg::EnableControls(BOOL bIsCameraReady)
{
...
void CBasicDemoByGenTLDlg::DisplayWindowInitial()
{
...
void CBasicDemoByGenTLDlg::OnBnClickedEnumIfButton()
{
    ...
    for (unsigned int i = 0; i < m_stIFList.nInterfaceNum; i++)
    {
        ...
        sprintf_s(strUserName, 256, "Interface[%d]:%s %s (%s) (%d)", i, pstIFInfo->chTLType, pstIFInfo->chInterfaceID, pstIFInfo->chDisplayName, pstIFInfo->nCtiIndex);
...
void CBasicDemoByGenTLDlg::OnBnClickedEnumDevButton()
{
	...
	if (m_ctrlInterfaceCombo.GetCurSel() < 0 || m_ctrlInterfaceCombo.GetCurSel() > m_stIFList.nInterfaceNum)
	{
		ShowErrorMsg(_TEXT("No Device"), 0);
		EnableControls(FALSE);
		return;
    ...
    for (unsigned int i = 0; i < m_stDevList.nDeviceNum; i++)
    {
        ...
        {
            sprintf_s(strUserName, 256, "Dev[%d]:%s %s (%s)", i, pstDeviceInfo->chUserDefinedName, pstDeviceInfo->chModelName,pstDeviceInfo->chSerialNumber);
        }
        else
        {
            sprintf_s(strUserName, 256, "Dev[%d]:%s %s (%s)", i, pstDeviceInfo->chVendorName, pstDeviceInfo->chModelName,pstDeviceInfo->chSerialNumber);
        }
...
int CBasicDemoByGenTLDlg::CloseDevice()
{
    m_bThreadState = FALSE;
    if (m_hGrabThread)
    {
...
int CBasicDemoByGenTLDlg::GetTriggerMode()
{
...
int CBasicDemoByGenTLDlg::SetTriggerMode()
{
...
int CBasicDemoByGenTLDlg::GetTriggerSource()
{
    MVCC_ENUMVALUE stEnumValue = {0};
...
int CBasicDemoByGenTLDlg::SetTriggerSource()
{
