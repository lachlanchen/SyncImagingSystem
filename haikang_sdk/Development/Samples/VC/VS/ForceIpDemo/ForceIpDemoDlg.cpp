...
void CForceIpDemoDlg::OnBnClickedSetIpButton()
{
    ...
    // ch:判断设备IP是否可达
    bool bAccessible = MV_CC_IsDeviceAccessible(m_stDevList.pDeviceInfo[nIndex], MV_ACCESS_Exclusive);
    if(bAccessible)
    {
        nRet = MV_GIGE_SetIpConfig(m_handle, MV_IP_CFG_STATIC);
        if (MV_OK != nRet)
        {
            ShowErrorMsg(TEXT("Set Ip config fail"), nRet);
        ...
        if (MV_OK != nRet)
        {
            ShowErrorMsg(TEXT("ForceIp fail"), nRet);
            MV_CC_DestroyHandle(m_handle);
            m_handle = NULL;
            return;
        }
    ...
    else
    {
        ...
        m_stDevList.pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.nCurrentIp = m_dwIpaddress;
        m_stDevList.pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.nCurrentSubNetMask = m_dwNetworkMask;
        m_stDevList.pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.nDefultGateWay = m_dwDefaultGateway;
        int nRet = MV_CC_CreateHandle(&m_handle, m_stDevList.pDeviceInfo[nIndex]);
        if (MV_OK != nRet)
        {
            ShowErrorMsg(TEXT("Create handle fail"), nRet);
    ...
    ShowErrorMsg(TEXT("ForceIp succeed"), 0);
...
void CForceIpDemoDlg::DisplayDeviceIp()
{
    ...
    int nNetIp1 = ((m_stDevList.pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.nNetExport & 0xff000000) >> 24);
    int nNetIp2 = ((m_stDevList.pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.nNetExport & 0x00ff0000) >> 16);
    int nNetIp3 = ((m_stDevList.pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.nNetExport & 0x0000ff00) >> 8);
    int nNetIp4 = (m_stDevList.pDeviceInfo[nIndex]->SpecialInfo.stGigEInfo.nNetExport & 0x000000ff);

    char chIPRange[128] = {0};
    wchar_t strIPRange[128] = {0};

    sprintf_s(chIPRange, 128, "%d.%d.%d.0---%d.%d.%d.255", nNetIp1, nNetIp2, nNetIp3, nNetIp1, nNetIp2, nNetIp3);
    MultiByteToWideChar(CP_ACP, 0, (LPCSTR)(chIPRange), -1, strIPRange, 128);
    GetDlgItem(IDC_STATIC_IP_RANGE)->SetWindowText(strIPRange);

    UpdateData(FALSE);
}

// ch:右上角退出 | en:Exit from upper right corner
void CForceIpDemoDlg::OnClose()
{
	PostQuitMessage(0);
	MV_CC_Finalize();
	CDialog::OnClose();
