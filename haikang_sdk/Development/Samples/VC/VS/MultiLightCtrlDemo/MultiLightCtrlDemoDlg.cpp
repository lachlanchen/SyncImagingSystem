    ...
    while(m_bThreadState)
    {
        ...
        if (nRet == MV_OK)
        {
            ...
            if(true == bHBFalg)
            {
                ...
                stHBParam.nSrcLen = stImageInfo.stFrameInfo.nFrameLen;
            ...
            {
                if (stImgReconstructionParam.nSrcDataLen / (m_nMultiLightNum-1) > nImageBufferSize)
                {
                    for (unsigned int i = 0; i < EXPOSURE_NUM; i++)
                    {
                        ...
                        pImageBufferList[i] = (unsigned char*)malloc(sizeof(unsigned char) * stImgReconstructionParam.nSrcDataLen / (m_nMultiLightNum-1));
                        if (NULL != pImageBufferList[i])
                        {
                            stImgReconstructionParam.stDstBufList[i].pBuf = pImageBufferList[i];
                            stImgReconstructionParam.stDstBufList[i].nBufSize = stImgReconstructionParam.nSrcDataLen / (m_nMultiLightNum-1);
                        }
                        else
                        {
                            return MV_E_RESOURCE;
                    ...
                    nImageBufferSize = stImgReconstructionParam.nSrcDataLen / (m_nMultiLightNum-1);
                }
