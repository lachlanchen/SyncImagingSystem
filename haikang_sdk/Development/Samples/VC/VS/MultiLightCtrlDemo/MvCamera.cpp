...
//ch:HB模式解码
int CMvCamera::HB_Decode(MV_CC_HB_DECODE_PARAM* pstDecodeParam)
{
    if (MV_NULL == pstDecodeParam)
    {
    ...
    int nRet = MV_CC_HB_Decode(m_hDevHandle, pstDecodeParam);
    if (MV_OK != nRet)
    {
        return nRet;
    }
...
int CMvCamera::ReconstructImage(MV_RECONSTRUCT_IMAGE_PARAM* pstReconstructParam)
{
	return MV_CC_ReconstructImage(m_hDevHandle, pstReconstructParam);
}
