...
int main()
{
    ...
    if (MvCamCtrlDll)
    {
        do 
        {
            ...
            nRet = DLLEnumDevices(MV_GIGE_DEVICE | MV_USB_DEVICE | MV_GENTL_CAMERALINK_DEVICE | MV_GENTL_CXP_DEVICE | MV_GENTL_XOF_DEVICE, &stDeviceList);
            if (MV_OK != nRet)
            {
		...
		DLL_Finalize DLLFinalize = (DLL_Finalize)GetProcAddress(MvCamCtrlDll, "MV_CC_Finalize");
		DLLFinalize();
