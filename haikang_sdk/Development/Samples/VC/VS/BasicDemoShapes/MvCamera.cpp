...
// ch:在显示图像上绘制矩形 | en:Draw rectangle on display image
int CMvCamera::DrawRect(MVCC_RECT_INFO* pRectInfo)
{
    return MV_CC_DrawRect(m_hDevHandle, pRectInfo);
}

// ch:在显示图像上绘制圆形 | en:Draw circle on display image
int CMvCamera::DrawCircle(MVCC_CIRCLE_INFO* pCircleInfo)
{
    return MV_CC_DrawCircle(m_hDevHandle, pCircleInfo);
}

// ch:在显示图像上绘制线条 | en:Draw line on display image
int CMvCamera::DrawLine(MVCC_LINES_INFO* pLinesInfo)
{
