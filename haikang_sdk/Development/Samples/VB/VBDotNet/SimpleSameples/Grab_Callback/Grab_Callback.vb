...
Module Grab_Callback
    ...
    Private Sub cbOutputdelegateFunc(ByVal pData As IntPtr, ByRef stFrameInfoEx As CCamera.MV_FRAME_OUT_INFO_EX, ByVal pUser As IntPtr)
        Console.WriteLine("Width:" + Convert.ToString(stFrameInfoEx.nWidth) + " Height:" + Convert.ToString(stFrameInfoEx.nHeight) + " FrameNum:" + Convert.ToString(stFrameInfoEx.nFrameNum))
    End Sub

    Sub Main()
        Dim nRet As Int32 = CCamera.MV_OK
        Dim stDeviceInfoList As CCamera.MV_CC_DEVICE_INFO_LIST = New CCamera.MV_CC_DEVICE_INFO_LIST
        Dim cbCallbackEx As CCamera.cbOutputExdelegate = New CCamera.cbOutputExdelegate(AddressOf cbOutputdelegateFunc)
        ...
        Do While (True)
            ...
            nRet = MyCamera.RegisterImageCallBackEx(cbCallbackEx, 0)
            If CCamera.MV_OK <> nRet Then
