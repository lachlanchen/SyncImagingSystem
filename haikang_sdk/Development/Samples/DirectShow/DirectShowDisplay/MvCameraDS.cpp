...
HRESULT CMvCameraDS::GetCameraControl()
{
    ...
    else
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                printf("Get CameraControl_Exposure[%d]\n", Val);
            }

            hr = pCameraControl->Set(CameraControl_Exposure, Val, Flags);
            if (SUCCEEDED(hr))
            {
                printf("Set CameraControl_Exposure[%d]\n", Val);
            }
        }
...
int CMvCameraDS::StartGrab()
{
    ...
    do 
    {
        ...
        // 创建渲染Filter
        CoCreateInstance(CLSID_VideoRenderer, NULL, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (LPVOID*) &m_pRenderFilter);
