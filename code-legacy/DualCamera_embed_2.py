...
class FrameCameraController:
    ...
    def __init__(self, status_callback=None):
        ...
        self.embedded_preview_thread = None
        self.command_queue = queue.Queue()
        self.frame_queue = queue.Queue(maxsize=5)
        self.embedded_frame_queue = queue.Queue(maxsize=1)  # Minimal queue for zero lag
        ...
        self.gui_update_callback = None  # For embedded preview
        ...
        try:
            ...
            while not self.frame_queue.empty():
                try:
                    ...
                    break
            while not self.embedded_frame_queue.empty():
                try:
                    self.embedded_frame_queue.get_nowait()
                except:
                    break
    ...
    def start_preview(self):
        ...
        if not self.show_preview:
            ...
            # Start embedded preview thread
            self.embedded_preview_thread = threading.Thread(target=self._embedded_preview_worker, daemon=True)
            self.embedded_preview_thread.start()
            return True
        return False
    ...
    def stop_preview(self):
        ...
        if self.show_preview:
            self.show_preview = False
            if self.embedded_preview_thread:
                self.embedded_preview_thread.join(timeout=2.0)
            if self.preview_thread:
                self.preview_thread.join(timeout=2.0)
            # Clear embedded preview
    ...
    def _create_external_preview(self):
        ...
        self.preview_thread = threading.Thread(target=self._external_preview_worker, daemon=True)
    ...
    def _embedded_preview_worker(self):
        """Embedded preview worker thread - ultra-optimized for zero lag"""
        self._notify_status("Frame preview started (embedded mode)")
        
        while not self.exit_flag and self.show_preview:
            try:
                frame = self.embedded_frame_queue.get(timeout=0.001)  # Ultra-fast timeout
                
                # Send frame to GUI update callback immediately (no checks)
                if self.gui_update_callback:
                    self.gui_update_callback(frame)
            ...
            if self.is_grabbing:
                ...
                if frame_out.pBufAddr and ret == 0:
                    ...
                    if self.show_preview:
                        ...
                        if img is not None:
                            # Send to external preview queue
                            try:
                                self.frame_queue.put_nowait(img)
                            except queue.Full:
                                pass
                            
                            # Ultra-aggressive frame dropping for embedded preview
                            if not (hasattr(self, 'external_preview_active') and self.external_preview_active):
                                # Clear queue completely and add only latest frame
                                while not self.embedded_frame_queue.empty():
                                    try:
                                        self.embedded_frame_queue.get_nowait()
                                    except queue.Empty:
                                        break
                                try:
                                    self.embedded_frame_queue.put_nowait(img)
                                except queue.Full:
                                    pass
...
class EventCameraController:
    ...
    def start_visualization(self):
        ...
        # Start embedded visualization thread
        self.embedded_viz_thread = threading.Thread(target=self._embedded_visualization_worker, daemon=True)
        self.embedded_viz_thread.start()
        return True
    ...
    def stop_visualization(self):
        ...
        if self.visualization_running:
            ...
            if self.embedded_viz_thread:
                self.embedded_viz_thread.join(timeout=3.0)
    ...
    def _create_external_visualization(self):
        ...
        self.external_preview_active = True
        self.viz_thread = threading.Thread(target=self._external_visualization_worker, daemon=True)
    ...
    def _embedded_visualization_worker(self):
        """Embedded visualization worker - ultra-optimized"""
        try:
            event_frame_gen = PeriodicFrameGenerationAlgorithm(
                sensor_width=self.width, sensor_height=self.height, 
                fps=25, palette=ColorPalette.Dark
                ...
                if not (hasattr(self, 'external_preview_active') and self.external_preview_active):
                    ...
                    while not self.embedded_frame_queue.empty():
                        try:
                            ...
                            break
                    try:
                        self.embedded_frame_queue.put_nowait(cd_frame)
                    except queue.Full:
                        pass
            
            event_frame_gen.set_output_callback(on_cd_frame_cb)
            
            # Start GUI update thread for embedded preview
            gui_thread = threading.Thread(target=self._embedded_gui_worker, daemon=True)
            gui_thread.start()
    ...
    def _embedded_gui_worker(self):
        """GUI worker thread for embedded visualization - ultra-optimized for zero lag"""
        while not self.should_exit and self.visualization_running:
            try:
                frame = self.embedded_frame_queue.get(timeout=0.001)  # Ultra-fast timeout
...
class DualCameraGUI:
    ...
    def __init__(self):
        ...
        # Set preview callbacks with thread-safe GUI updates
        self.frame_camera.set_gui_update_callback(self._safe_update_frame_preview)
        self.event_camera.set_gui_update_callback(self._safe_update_event_preview)
        
        # Auto-scan for cameras
        self.scan_cameras()
    
    def create_widgets(self):
        """Create GUI widgets with draggable splitter and centered preview areas"""
        ...
        # Frame camera preview (top right) - Use Canvas for better centering
        ...
        self.frame_preview_canvas = tk.Canvas(self.frame_preview_frame, bg='black', highlightthickness=0)
        self.frame_preview_canvas.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
        
        # Initial text for frame preview
        self.frame_preview_text = self.frame_preview_canvas.create_text(
            0, 0, text="Frame preview will appear here\nStart preview to see camera feed",
            fill='white', font=('Arial', 12), anchor='center'
        )
        
        # Event camera preview (bottom right) - Use Canvas for better centering
        ...
        self.event_preview_canvas = tk.Canvas(self.event_preview_frame, bg='black', highlightthickness=0)
        self.event_preview_canvas.pack(fill=tk.BOTH, expand=True, padx=2, pady=2)
        
        # Initial text for event preview
        self.event_preview_text = self.event_preview_canvas.create_text(
            0, 0, text="Event preview will appear here\nStart visualization to see camera feed",
            fill='white', font=('Arial', 12), anchor='center'
        )
        
        # Bind canvas resize events to center content
        self.frame_preview_canvas.bind('<Configure>', self._on_frame_canvas_resize)
        self.event_preview_canvas.bind('<Configure>', self._on_event_canvas_resize)
    ...
    def _on_frame_canvas_resize(self, event):
        """Handle frame canvas resize to center content"""
        canvas = event.widget
        canvas_width = canvas.winfo_width()
        canvas_height = canvas.winfo_height()
        
        # Center the text/image
        if hasattr(self, 'frame_preview_text'):
            canvas.coords(self.frame_preview_text, canvas_width//2, canvas_height//2)
        
        if hasattr(self, 'frame_preview_image_id'):
            canvas.coords(self.frame_preview_image_id, canvas_width//2, canvas_height//2)
    
    def _on_event_canvas_resize(self, event):
        """Handle event canvas resize to center content"""
        canvas = event.widget
        canvas_width = canvas.winfo_width()
        canvas_height = canvas.winfo_height()
        
        # Center the text/image
        if hasattr(self, 'event_preview_text'):
            canvas.coords(self.event_preview_text, canvas_width//2, canvas_height//2)
        
        if hasattr(self, 'event_preview_image_id'):
            canvas.coords(self.event_preview_image_id, canvas_width//2, canvas_height//2)
    
    def _safe_update_frame_preview(self, frame):
        """Thread-safe frame preview update with zero lag optimization"""
        if not self._frame_update_pending:
            self._frame_update_pending = True
            self.root.after_idle(lambda: self._do_frame_preview_update(frame))
    
    def _safe_update_event_preview(self, frame):
        """Thread-safe event preview update with zero lag optimization"""
        if not self._event_update_pending:
            self._event_update_pending = True
            self.root.after_idle(lambda: self._do_event_preview_update(frame))
    
    def _do_frame_preview_update(self, frame):
        """Actual frame preview update with immediate flag reset"""
        self._frame_update_pending = False
        self.update_frame_preview(frame)
    
    def _do_event_preview_update(self, frame):
        """Actual event preview update with immediate flag reset"""
        self._event_update_pending = False
        self.update_event_preview(frame)
    
    def update_frame_preview(self, frame):
        """Update frame camera preview - ultra-optimized with Canvas centering"""
        try:
            if frame is not None and PIL_AVAILABLE:
                ...
                if canvas_width > 50 and canvas_height > 50:
                    # Hide text
                    if hasattr(self, 'frame_preview_text'):
                        canvas.itemconfig(self.frame_preview_text, state='hidden')
                    
                    # Calculate optimal size that fills the canvas
                    h, w = frame.shape[:2]
                    ...
                    if new_width > 0 and new_height > 0:
                        ...
                        # Update or create image on canvas (centered)
                        if hasattr(self, 'frame_preview_image_id'):
                            canvas.itemconfig(self.frame_preview_image_id, image=tk_image)
                        else:
                            self.frame_preview_image_id = canvas.create_image(
                                canvas_width//2, canvas_height//2, image=tk_image, anchor='center'
                            )
                        
                        # Keep reference to prevent garbage collection
                        self.frame_preview_image = tk_image
            
            elif frame is None:
                # Clear preview and show text
                canvas = self.frame_preview_canvas
                if hasattr(self, 'frame_preview_image_id'):
                    canvas.delete(self.frame_preview_image_id)
                    delattr(self, 'frame_preview_image_id')
                if hasattr(self, 'frame_preview_text'):
                    canvas.itemconfig(self.frame_preview_text, state='normal')
                    canvas.itemconfig(self.frame_preview_text, text="Frame preview stopped\nStart preview to see camera feed")
                
        except Exception as e:
            pass
    
    def update_event_preview(self, frame):
        """Update event camera preview - ultra-optimized with Canvas centering"""
        try:
            if frame is not None and PIL_AVAILABLE:
                canvas = self.event_preview_canvas
                canvas_width = canvas.winfo_width()
                canvas_height = canvas.winfo_height()
                ...
                if canvas_width > 50 and canvas_height > 50:
                    # Hide text
                    if hasattr(self, 'event_preview_text'):
                        canvas.itemconfig(self.event_preview_text, state='hidden')
                    ...
                    if new_width > 0 and new_height > 0:
                        ...
                        # Update or create image on canvas (centered)
                        if hasattr(self, 'event_preview_image_id'):
                            canvas.itemconfig(self.event_preview_image_id, image=tk_image)
                        else:
                            self.event_preview_image_id = canvas.create_image(
                                canvas_width//2, canvas_height//2, image=tk_image, anchor='center'
                            )
                        
                        # Keep reference to prevent garbage collection
                        self.event_preview_image = tk_image
            
            elif frame is None:
                # Clear preview and show text
                canvas = self.event_preview_canvas
                if hasattr(self, 'event_preview_image_id'):
                    canvas.delete(self.event_preview_image_id)
                    delattr(self, 'event_preview_image_id')
                if hasattr(self, 'event_preview_text'):
                    canvas.itemconfig(self.event_preview_text, state='normal')
                    canvas.itemconfig(self.event_preview_text, text="Event preview stopped\nStart visualization to see camera feed")
