...
class FrameCameraController:
    ...
    def _preview_worker(self):
        ...
        if WINDOWS_AVAILABLE and self.preview_always_on_top:
        ...
        while not self.exit_flag and self.show_preview:
            try:
                ...
                # Add overlay text showing always on top status
                overlay_frame = frame.copy()
                status_text = f"Always on top: {'ON' if self.preview_always_on_top else 'OFF'} (Right-click/T to toggle)"
...
class EventCameraController:
    """Modified version of EventCameraRecorder for GUI integration - SIMPLIFIED CONNECTION"""
    ...
    def __init__(self, status_callback=None):
        ...
        # Window reference for proper cleanup and always on top
        self.window = None
        self.window_always_on_top = False
    
    def find_cameras(self):
        """Find available event cameras - SIMPLE APPROACH"""
        if not EVENT_CAMERA_AVAILABLE:
            return []
        
        cameras = []
        try:
            # Simple approach - just try to detect one camera
            device = initiate_device("")
            if device is not None:
                cameras.append((0, "Event Camera", device))
                # Don't keep the device, just close it
                device = None
        except Exception as e:
            self._notify_status(f"Event camera scan: {e}")
        
        return cameras
    
    def connect_camera(self, device=None):
        """Connect to event camera - SIMPLE APPROACH FROM WORKING CODE"""
        if not EVENT_CAMERA_AVAILABLE:
            return False
        
        try:
            # Simple disconnect first
            self.disconnect_camera()
            
            # Simple connection - exactly like the working code
            self.device = initiate_device("")
            
            if self.device is None:
                self._notify_status("Event camera connection failed: device is None")
                return False
    ...
    def disconnect_camera(self):
        """Disconnect camera - SIMPLE APPROACH"""
        self.stop_all()
        if self.device:
            self.device = None
        self.mv_iterator = None
    ...
    def _visualization_thread(self):
        ...
        try:
            ...
            with MTWindow(title="Event Camera Preview", width=self.width, height=self.height,
                ...
                # Set initial always on top state
                if WINDOWS_AVAILABLE and self.window_always_on_top:
                    time.sleep(0.2)  # Give window time to be created
                    set_window_always_on_top("Event Camera Preview", True)
...
class DualCameraGUI:
    ...
    def create_always_on_top_section(self, parent):
        ...
        info_text = "Preview Window Controls:"
        if not WINDOWS_AVAILABLE:
    ...
    def connect_event_camera(self):
        """Connect to event camera - SIMPLE APPROACH"""
