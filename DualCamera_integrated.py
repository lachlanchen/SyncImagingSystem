...
def cv2_to_tkinter(cv_image, max_width=400, max_height=300):
...
class FrameCameraController:
    """Modified version of CameraRecorder for GUI integration with embedded preview"""
    
    def __init__(self, status_callback=None, preview_widget=None):
        self.cam = None
        ...
        self.preview_widget = preview_widget  # tkinter widget for embedded preview
    ...
    def _preview_worker(self):
        """Preview worker thread for embedded widget"""
        ...
        while not self.exit_flag and self.show_preview:
            try:
                ...
                if self.preview_widget and frame is not None:
                    ...
                    cv2.putText(overlay_frame, f"Frame: {transform_text}", (10, 30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                    if self.is_recording:
                        cv2.putText(overlay_frame, "RECORDING", (10, 60), 
                                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                    
                    # Convert to tkinter format and update widget
                    tk_image = cv2_to_tkinter(overlay_frame)
                    if tk_image:
                        try:
                            self.preview_widget.configure(image=tk_image, text="")
                            self.preview_widget.image = tk_image  # Keep a reference
                        except:
...
class EventCameraController:
    """Modified version of EventCameraRecorder for GUI integration with embedded preview"""
    ...
    def _visualization_thread(self):
        """Thread for event visualization - embedded mode"""
        try:
            ...
            def on_cd_frame_cb(ts, cd_frame):
                ...
                if self.is_recording:
                    cv2.putText(overlay_frame, "Event: RECORDING", (10, 30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                else:
                    cv2.putText(overlay_frame, "Event: Live", (10, 30), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                
                try:
                    frame_queue.put_nowait(overlay_frame)
                except queue.Full:
                    pass
            ...
            while not self.should_exit and self.visualization_running:
                try:
                    ...
                    try:
                        cd_frame = frame_queue.get_nowait()
                        if self.preview_widget and cd_frame is not None:
                            tk_image = cv2_to_tkinter(cd_frame)
                            if tk_image:
                                try:
                                    self.preview_widget.configure(image=tk_image, text="")
                                    self.preview_widget.image = tk_image  # Keep a reference
...
class DualCameraGUI:
    """Main GUI application with embedded previews"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Dual Camera Control System - Embedded Previews")
        
        # Get screen dimensions for optimal window layout
        ...
        # Create preview widgets first (before camera controllers)
        self.frame_preview_widget = None
        self.event_preview_widget = None
        
        # Camera controllers (will be set after creating preview widgets)
    ...
    def create_widgets(self):
        """Create GUI widgets with embedded previews"""
        ...
        main_paned.add(controls_frame, weight=60)  # 60% for controls
        
        # Right side - previews
        preview_frame = ttk.Frame(main_paned)
        main_paned.add(preview_frame, weight=40)  # 40% for previews
        ...
        # Initialize camera controllers with preview widgets
        self.frame_camera = FrameCameraController(self.update_status, self.frame_preview_widget)
        self.event_camera = EventCameraController(self.update_status, self.event_preview_widget)
    ...
    def create_preview_section(self, parent):
        """Create embedded preview section"""
        ...
        self.frame_preview_widget.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        self.event_preview_widget.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def create_status_section(self, parent):
        ...
        self.status_text = tk.Text(parent, height=8, state=tk.DISABLED)
