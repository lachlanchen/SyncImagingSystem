...
class FrameCameraController:
    ...
    def _preview_worker(self):
        ...
        # Position window separately from main GUI
        window_width = 640
        window_height = 480
        window_x = 100
        window_y = 100
...
class EventCameraController:
    ...
    def _visualization_thread(self):
        ...
        try:
            ...
            # Create window separately from main GUI
            window_width = min(self.width, 800)
            window_height = min(self.height, 600)
            ...
            with MTWindow(title="Event Camera Preview", width=window_width, height=window_height,
                ...
                # Position window separately from main GUI using Windows API
                if WINDOWS_AVAILABLE:
                    def position_window():
                        ...
                        try:
                            ...
                            if hwnd:
                                x = 750  # Position away from main GUI
                                y = 100
                                width = window_width
...
class DualCameraGUI:
    ...
    def __init__(self):
        ...
        # Set main window size and position - compact control window
        window_width = 800
        window_height = 900  # Tall enough for all controls
    ...
    def create_widgets(self):
        """Create GUI widgets with simplified layout"""
        ...
        # Status section - MOVED TO BOTTOM OF CONTROLS
        status_section = ttk.LabelFrame(scrollable_frame, text="Status Log")
        status_section.pack(fill=tk.X, padx=5, pady=5)  # Changed from BOTH to X
    ...
    def create_always_on_top_section(self, parent):
        ...
        ttk.Label(parent, text="• Frame Preview: Right-click or press 'T' in preview window\n• Event Preview: Press 'T' in preview window\n• Preview windows open as separate pop-ups", 
                 font=('TkDefaultFont', 8)).pack(padx=5, pady=2)
    
    def create_status_section(self, parent):
        """Create status section - now part of main controls"""
        # Make status section smaller since it's now in the main controls area
        self.status_text = tk.Text(parent, height=8, state=tk.DISABLED)  # Reduced height
