...
class FrameCameraController:
    ...
    def __init__(self, status_callback=None):
        ...
        self.embedded_frame_queue = queue.Queue(maxsize=1)  # Very small queue for low latency
    ...
    def _embedded_preview_worker(self):
        """Embedded preview worker thread - optimized for minimal lag"""
        self._notify_status("Frame preview started (embedded mode)")
        
        while not self.exit_flag and self.show_preview:
            try:
                frame = self.embedded_frame_queue.get(timeout=0.02)  # Very fast timeout
                
                # Send frame to GUI update callback immediately
                if self.gui_update_callback and not (hasattr(self, 'external_preview_active') and self.external_preview_active):
                ...
                if frame_out.pBufAddr and ret == 0:
                    ...
                    if self.show_preview:
                        ...
                        if img is not None:
                            ...
                            # Send to embedded preview queue with aggressive frame dropping
                            try:
                                # Always try to drop old frame first for low latency
                                try:
                                    self.embedded_frame_queue.get_nowait()
                                except queue.Empty:
                                    pass
...
class EventCameraController:
    ...
    def _embedded_visualization_worker(self):
        """Embedded visualization worker - separate from external"""
        try:
            event_frame_gen = PeriodicFrameGenerationAlgorithm(
                sensor_width=self.width, sensor_height=self.height, 
                fps=25, palette=ColorPalette.Dark
            ...
            def on_cd_frame_cb(ts, cd_frame):
                # Send frame to embedded queue with aggressive frame dropping
    ...
    def _embedded_gui_worker(self):
        """GUI worker thread for embedded visualization - optimized for minimal lag"""
        while not self.should_exit and self.visualization_running:
            try:
                frame = self.embedded_frame_queue.get(timeout=0.02)  # Very fast timeout
...
class DualCameraGUI:
    ...
    def create_widgets(self):
        ...
        # Frame camera preview (top right)
        ...
        # Event camera preview (bottom right)
    ...
    def _safe_update_frame_preview(self, frame):
        """Thread-safe frame preview update with minimal throttling"""
        # Very lightweight throttling - only check if we're already updating
        if not getattr(self, '_frame_update_pending', False):
    ...
    def _safe_update_event_preview(self, frame):
        """Thread-safe event preview update with minimal throttling"""
        # Very lightweight throttling - only check if we're already updating
        if not getattr(self, '_event_update_pending', False):
    ...
    def update_frame_preview(self, frame):
        """Update frame camera preview in GUI - ultra-optimized for minimal lag"""
        try:
            if frame is not None and PIL_AVAILABLE:
                # Get preview label dimensions (cached for 2 seconds)
                if not hasattr(self, '_frame_label_dims') or time.time() - getattr(self, '_frame_label_dims_time', 0) > 2.0:
                    self._frame_label_dims = (self.frame_preview_label.winfo_width(), self.frame_preview_label.winfo_height())
                    self._frame_label_dims_time = time.time()
                
                label_width, label_height = self._frame_label_dims
                
                if label_width > 50 and label_height > 50:
                    # Fast conversion - skip RGB conversion, resize first
                    h, w = frame.shape[:2]
                    
                    # Calculate size with more aggressive limits
                    max_width = min(label_width - 10, 480)  # Even smaller for speed
                    max_height = min(label_height - 10, 360)
                    ...
                    if new_width > 0 and new_height > 0:
                        # Resize first (in BGR), then convert to RGB
                        frame_resized = cv2.resize(frame, (new_width, new_height), interpolation=cv2.INTER_NEAREST)  # Fastest interpolation
                        ...
                        self.frame_preview_label.image = tk_image
            elif frame is None:
    ...
    def update_event_preview(self, frame):
        """Update event camera preview in GUI - ultra-optimized for minimal lag"""
        try:
            if frame is not None and PIL_AVAILABLE:
                # Get preview label dimensions (cached for 2 seconds)
                if not hasattr(self, '_event_label_dims') or time.time() - getattr(self, '_event_label_dims_time', 0) > 2.0:
                    self._event_label_dims = (self.event_preview_label.winfo_width(), self.event_preview_label.winfo_height())
                    self._event_label_dims_time = time.time()
                
                label_width, label_height = self._event_label_dims
                ...
                if label_width > 50 and label_height > 50:
                    ...
                    if new_width > 0 and new_height > 0:
                        ...
                        self.event_preview_label.image = tk_image
            elif frame is None:
