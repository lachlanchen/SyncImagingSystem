...
sys.path.append(os.path.join(BASE, "haikang_sdk", "Python", "BasicDemo"))
...
class PreviewWindow(QMainWindow):
    """Preview window that runs in the Qt thread"""
    ...
    def __init__(self):
        ...
        # Disable all controls except the display widget
        self.ui.groupInit.setEnabled(False)
        self.ui.groupGrab.setEnabled(False)
        self.ui.groupParam.setEnabled(False)
        self.ui.ComboDevices.setEnabled(False)
        
        # Get the window handle for display
        self.display_window_id = self.ui.widgetDisplay.winId()
        
    def get_window_id(self):
        """Get the window ID for camera display"""
        return self.display_window_id

class PreviewThread(QThread):
    """Thread to handle Qt GUI and preview window"""
    window_ready = pyqtSignal(int)  # Signal with window ID
    ...
    def run(self):
        ...
        # Emit the window ID
        window_id = self.window.get_window_id()
        self.window_ready.emit(window_id)
...
class CameraRecorder:
    ...
    def __init__(self, camera_handle, device_info):
        ...
        self.frame_queue = queue.Queue(maxsize=2)  # For preview frames
    ...
    def on_window_ready(self, window_id):
        """Called when preview window is ready"""
        self.preview_window_id = window_id
        print(f"Preview window ready (ID: {window_id})")
    ...
    def recording_worker(self):
        ...
        display_frame_info = MV_DISPLAY_FRAME_INFO()
        memset(byref(display_frame_info), 0, sizeof(MV_DISPLAY_FRAME_INFO))
        ...
        while not self.exit_flag:
            ...
            try:
                ...
                if cmd == 'START_RECORDING':
                    ...
                    print("\nStarted preview display")
                
                elif cmd == 'STOP_PREVIEW':
                    self.show_preview = False
                    print("\nStopped preview display")
            ...
            if self.is_grabbing:
                ...
                if frame_out.pBufAddr and ret == 0:
                    ...
                    # If preview is enabled and window is ready, display the frame
                    if self.show_preview and self.preview_window_id:
                        display_frame_info.hWnd = int(self.preview_window_id)
                        display_frame_info.nWidth = frame_out.stFrameInfo.nWidth
                        display_frame_info.nHeight = frame_out.stFrameInfo.nHeight
                        display_frame_info.enPixelType = frame_out.stFrameInfo.enPixelType
                        display_frame_info.pData = frame_out.pBufAddr
                        display_frame_info.nDataLen = frame_out.stFrameInfo.nFrameLen
                        
                        ret = self.cam.MV_CC_DisplayOneFrame(display_frame_info)
                        if ret == 0:
                            with self.stats_lock:
    ...
    def start_preview(self):
        """Start the preview window and thread"""
        if not self.preview_thread:
            self.preview_thread = PreviewThread()
            self.preview_thread.window_ready.connect(self.on_window_ready)
            self.preview_thread.start()
            # Wait a bit for window to be ready
            time.sleep(0.5)
        
        if self.is_grabbing:
            self.command_queue.put('START_PREVIEW')
        else:
            print("Start grabbing first before preview")
    
    def stop_preview(self):
        """Stop the preview display"""
        self.command_queue.put('STOP_PREVIEW')
        
        # Stop the preview thread
        if self.preview_thread:
            self.preview_thread.stop()
            self.preview_thread.wait(2000)  # Wait up to 2 seconds
            self.preview_thread = None
            self.preview_window_id = None
