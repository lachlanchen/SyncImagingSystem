...
class FrameCameraController:
    ...
    def __init__(self, status_callback=None):
        ...
        self.embedded_frame_queue = queue.Queue(maxsize=3)  # Separate queue for embedded preview
    ...
    def _embedded_preview_worker(self):
        ...
        while not self.exit_flag and self.show_preview:
            try:
                frame = self.embedded_frame_queue.get(timeout=0.1)
                ...
                if frame_out.pBufAddr and ret == 0:
                    ...
                    if self.show_preview:
                        ...
                        if img is not None:
                            ...
                            # Send to embedded preview queue
                            try:
...
class DualCameraGUI:
    ...
    def _safe_update_frame_preview(self, frame):
        ...
        self.root.after_idle(lambda: self.update_frame_preview(frame))
    
    def _safe_update_event_preview(self, frame):
        """Thread-safe event preview update"""
        self.root.after_idle(lambda: self.update_event_preview(frame))
    
    def update_frame_preview(self, frame):
        """Update frame camera preview in GUI - called from main thread"""
        try:
    ...
    def update_event_preview(self, frame):
        """Update event camera preview in GUI - called from main thread"""
        try:
            if frame is not None and PIL_AVAILABLE:
                # Get preview label dimensions
