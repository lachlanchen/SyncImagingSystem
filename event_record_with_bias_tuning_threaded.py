...
class EventCameraRecorder:
    ...
    def show_bias_values(self):
        """Display current bias values"""
        bias_interface = self.device.get_i_ll_biases()
        if bias_interface is None:
            print("Cannot access bias interface")
            return
        ...
        try:
            all_biases = bias_interface.get_all_biases()
            print("  Contrast:")
            print(f"    bias_diff: {all_biases.get('bias_diff', 'N/A')}")
            print(f"    bias_diff_off: {all_biases.get('bias_diff_off', 'N/A')}")
            print(f"    bias_diff_on: {all_biases.get('bias_diff_on', 'N/A')}")
            print("  Bandwidth:")
            print(f"    bias_fo: {all_biases.get('bias_fo', 'N/A')}")
            print(f"    bias_hpf: {all_biases.get('bias_hpf', 'N/A')}")
            print("  Advance:")
            print(f"    bias_refr: {all_biases.get('bias_refr', 'N/A')}")
        except Exception as e:
            print(f"Error reading bias values: {e}")
    
    def update_bias_value(self, bias_name, value):
        """Update a specific bias value"""
        bias_interface = self.device.get_i_ll_biases()
        if bias_interface is None:
            print("Cannot access bias interface")
            return False
        ...
        try:
            ...
            # Set the bias
            bias_interface.set(bias_name, int_value)
            
            # Verify it was set
            current_value = bias_interface.get(bias_name)
            print(f"Set {bias_name} to {current_value}")
            return True
            
        except ValueError:
            print(f"Invalid value: {value}. Please enter an integer.")
            return False
        except Exception as e:
            print(f"Error setting {bias_name}: {e}")
            return False
    ...
    def control_loop(self):
        ...
        print("  bias - Show current bias values")
        print("  set <bias_name> <value> - Set a bias value")
        print("    Example: set bias_diff_off 150")
        print("    Available: bias_diff, bias_diff_off, bias_diff_on,")
        print("               bias_fo, bias_hpf, bias_refr")
        ...
        while not self.should_exit:
            try:
                ...
                if command == "start":
                    ...
                    self.show_bias_values()
                elif command == "set":
                    if len(parts) < 3:
                        print("Usage: set <bias_name> <value>")
                        print("Example: set bias_diff_off 150")
                    else:
                        bias_name = parts[1]
                        value = parts[2]
                        self.update_bias_value(bias_name, value)
                elif command == "help":
                    print("\nCommands:")
                    print("  start - Start recording")
                    print("  stop  - Stop recording")
                    ...
                    print("  bias - Show current bias values")
                    print("  set <bias_name> <value> - Set a bias value")
                    print("    Available bias names:")
                    print("      bias_diff (contrast difference)")
                    print("      bias_diff_off (negative event threshold)")
                    print("      bias_diff_on (positive event threshold)")
                    print("      bias_fo (cutoff frequency)")
                    print("      bias_hpf (high pass filter)")
                    print("      bias_refr (refractory period)")
                    print("  help - Show this help")
                    print("  exit - Exit program\n")
                elif command == "exit":
                    print("Exiting...")
