...
class FrameCameraController:
    ...
    def _preview_worker(self):
        """Preview worker thread - now uses embedded preview"""
        ...
        while not self.exit_flag and self.show_preview:
            try:
                ...
                # Send frame to embedded preview
...
class EventCameraController:
    ...
    def _visualization_thread(self):
        """Thread for event visualization - now uses embedded preview"""
        try:
            event_frame_gen = PeriodicFrameGenerationAlgorithm(
                sensor_width=self.width, sensor_height=self.height, 
                fps=25, palette=ColorPalette.Dark
            ...
            def on_cd_frame_cb(ts, cd_frame):
                # Send frame to embedded preview
                if self.preview_callback:
                    self.preview_callback(cd_frame)
...
class DualCameraGUI:
    ...
    def __init__(self):
        ...
        # Use full screen width now that previews are embedded
    ...
    def create_widgets(self):
        """Create GUI widgets with embedded preview areas"""
        # Create main horizontal layout: controls on left, previews on right
        ...
        self.event_preview_frame = ttk.LabelFrame(preview_frame, text="Event Camera Preview")
        self.event_preview_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        ...
        self.frame_preview_frame = ttk.LabelFrame(preview_frame, text="Frame Camera Preview")
        self.frame_preview_frame.pack(fill=tk.BOTH, expand=True, pady=(5, 0))
        ...
        # Set preview area callbacks for camera controllers
    ...
    def update_frame_preview(self, frame):
        """Update frame camera preview in GUI"""
        try:
    ...
    def update_event_preview(self, frame):
        """Update event camera preview in GUI"""
        try:
            if frame is not None and PIL_AVAILABLE:
                # Get preview label dimensions
