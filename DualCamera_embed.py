...
class FrameCameraController:
    ...
    def _embedded_preview_worker(self):
        """Embedded preview worker thread - separate from external preview"""
        self._notify_status("Frame preview started (embedded mode)")
        
        while not self.exit_flag and self.show_preview:
            try:
                frame = self.embedded_frame_queue.get(timeout=0.05)  # Faster timeout for responsiveness
                
                # Send frame to GUI update callback (thread-safe)
                ...
                # Small delay to limit frame rate and reduce CPU usage
...
class EventCameraController:
    ...
    def _embedded_gui_worker(self):
        """GUI worker thread for embedded visualization"""
        while not self.should_exit and self.visualization_running:
            try:
                frame = self.embedded_frame_queue.get(timeout=0.05)  # Faster timeout for responsiveness
...
class DualCameraGUI:
    ...
    def _safe_update_frame_preview(self, frame):
        """Thread-safe frame preview update with throttling"""
        # Skip if GUI update is already pending
        if not hasattr(self, '_frame_update_pending'):
    ...
    def _safe_update_event_preview(self, frame):
        """Thread-safe event preview update with throttling"""
        # Skip if GUI update is already pending
        if not hasattr(self, '_event_update_pending'):
    ...
    def _do_frame_preview_update(self, frame):
        """Actual frame preview update with pending flag reset"""
        self.update_frame_preview(frame)
        self._frame_update_pending = False
    
    def _do_event_preview_update(self, frame):
        """Actual event preview update with pending flag reset"""
        self.update_event_preview(frame)
        self._event_update_pending = False
    
    def update_frame_preview(self, frame):
        """Update frame camera preview in GUI - optimized for speed"""
        try:
            if frame is not None and PIL_AVAILABLE:
                # Get preview label dimensions (cached)
                if not hasattr(self, '_frame_label_dims') or time.time() - getattr(self, '_frame_label_dims_time', 0) > 1.0:
                ...
                if label_width > 50 and label_height > 50:  # Valid dimensions with margin
                    # Convert OpenCV frame to tkinter format (optimized)
                    ...
                    target_width = min(label_width - 10, 640)  # Limit max size for speed
                    target_height = min(label_height - 10, 480)
    ...
    def update_event_preview(self, frame):
        """Update event camera preview in GUI - optimized for speed"""
        try:
            if frame is not None and PIL_AVAILABLE:
                # Get preview label dimensions (cached)
                if not hasattr(self, '_event_label_dims') or time.time() - getattr(self, '_event_label_dims_time', 0) > 1.0:
                ...
                if label_width > 50 and label_height > 50:  # Valid dimensions with margin
                    ...
                    if new_width > 0 and new_height > 0:
                        ...
                        # Convert BGR to RGB for PIL (optimized)
