...
class DualCameraGUI:
    ...
    def __init__(self):
        ...
        # Set main window to fixed size that fits the controls (not half screen)
        # This prevents the window from expanding and overlapping controls
        main_width = 600  # Fixed width suitable for controls
        main_height = screen_height - 100  # Full height minus taskbar
        self.root.geometry(f"{main_width}x{main_height}+0+0")
        
        # Prevent window resizing to avoid layout issues
        self.root.resizable(False, True)  # Allow vertical resize only
        
        # Store screen info for camera preview positioning
        # Calculate right side position based on main window width, not screen half
        preview_start_x = main_width + 10  # Start previews after main window with small gap
        preview_width = screen_width - preview_start_x - 10  # Use remaining screen width
        
        self.screen_info = {
            'width': screen_width,
            'height': screen_height,
            'right_half_x': preview_start_x,
            'right_half_width': preview_width,
            'top_half_height': (screen_height - 100) // 2,
            'bottom_half_y': (screen_height - 100) // 2 - 20
        }
